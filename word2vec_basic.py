...
def build_model(sess, graph, loss_model):
	...
	with graph.as_default():
		...
		with tf.device('/cpu:0'):
			...
			sm_weights = tf.Variable(tf.truncated_normal([vocabulary_size, embedding_size],
...
def train(sess, model, data, dictionary, batch_size, num_skips, skip_window, max_num_steps, checkpoint_step, loss_model):
...
if __name__ == '__main__':
	...
	num_sampled  = 50	# reduced number of negatice samples
	...
	with tf.Session(graph=graph) as sess:
		...
		#suffix = str(batch_size)+"_"+str(embedding_size)+"_"+str(skip_window)+"_"+str(num_skips)+"_"+str(max_num_steps)
		#loss_model = loss_model + "_" + suffix
